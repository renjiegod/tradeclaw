...
**能力边界（免费源）**：以日线为主（baostock/akshare 提供分钟聚合 K 线，均为非实时历史查询）；无实盘账户 / 持仓 / 下单（执行仍是模拟盘）；实时行情、盘中监控、实盘交易需要梯度三的 QMT。有 [Tushare Pro](https://tushare.pro) token 时，设 `TUSHARE_TOKEN` 或 `data.tushare.token` 即自动纳入降级链；如走非官方 / 自建的 Tushare 兼容网关，另设 `TUSHARE_URL` 或 `data.tushare.url`，留空则使用官方默认地址。
