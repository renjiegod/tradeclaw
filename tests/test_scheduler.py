...
class SchedulerTests(unittest.IsolatedAsyncioTestCase):
    async def test_scheduler_runs_only_running_instances(self):
        ...
        inst_a = CycleTask(config=CycleTaskConfig(name="a", mode="paper"), worker=worker_a)
        inst_b = CycleTask(config=CycleTaskConfig(name="b", mode="paper"), worker=worker_b)

        scheduler.register(inst_a)
        scheduler.register(inst_b)

        scheduler.start(inst_a.task_id)
        await scheduler.tick_once()

        self.assertEqual(worker_a.cycles, 1)
        self.assertEqual(worker_b.cycles, 0)

        scheduler.start(inst_b.task_id)
        await scheduler.tick_once()
