...
4. `build_universe`
5. `run_strategy`（单入口：`TradingStrategyProtocol.execute_cycle`，内含信号生成与确定性审核）
6. `build_order_intents`
7. `run_risk_checks`
8. `await_approval_if_needed`
9. `dispatch_orders`
10. `sync_fills_and_positions`
11. `persist_trace_and_metrics`
...
该方案适合作为 Doyoutrade 的首版系统设计基线，并可在后续平滑演进为更强的多策略、多账户、多通道、多实例协同平台。